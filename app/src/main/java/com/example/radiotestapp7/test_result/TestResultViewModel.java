...
public class TestResultViewModel extends ViewModel {
    ...
    private long calculateUploadThruput() {
        ...
        long finisBytes = 0L;
        for (Log l : logList){
            if (l.getUlThrputStartTime() > 0L) startTime = l.getUlThrputStartTime();
            if (l.getUlThrputStartBytes() > 0L) startBytes = l.getUlThrputStartBytes();
            if (l.getUlThrputFinishTime() > 0L) finishTime = l.getUlThrputFinishTime();
            if (l.getUlThrputFinishBytes() > 0L) finisBytes = l.getUlThrputFinishBytes();
            if (l.getLogState() == EState.UPLOAD_TEST) {
                try{
                    ...
                    if (l.getUlThrputFinishTime() > 0L) finishTime = l.getUlThrputFinishTime();
                    if (l.getUlThrputFinishBytes() > 0L) finisBytes = l.getUlThrputFinishBytes();
                    Logger.d("UPLINK raws " + avgThrput + " " + startTime + " " + finishTime + " " + startBytes + " " + finisBytes);
                } catch (Exception exception ){ Logger.d("TestResultData upload " + exception.getMessage()); }
            }
        }

        if (uploadSR == 0.0 ) avgThrput = 0L;
        else {
            avgThrput = (finisBytes - startBytes) * 8L / 1000L/ (finishTime / 1000L - startTime / 1000L);
            Logger.d("UPLINK raws " + avgThrput + " " + startTime + " " + finishTime + " " + startBytes + " " + finisBytes);
        }
        Logger.d("Avg " + avgThrput);
        return avgThrput;
    }
        ...
        for (Log l : logList){
            if (l.getLogState() == EState.DOWNLOAD_TEST) {
                try{
                    ...
                    k++;
                    if (l.getDlThrputStartTime() > 0L) startTime = l.getDlThrputStartTime();
                    if (l.getDlThrputStartBytes() > 0L) startBytes = l.getDlThrputStartBytes();
                    if (l.getDlThrputFinishTime() > 0L) finishTime = l.getDlThrputFinishTime();
                    if (l.getDlThrputFinishBytes() > 0L) finisBytes = l.getDlThrputFinishBytes();
                    Logger.d("AvgDL " + avgThrput + " " + finisBytes + " " + finishTime + " " + startBytes + " " + startTime);
                } catch (Exception exception ){ Logger.d("TestResultData download " + exception.getMessage()); }
            }
            Logger.d("Avg " + l.getUlThrputStartBytes() + " " + l.getDlThrputFinishBytes());
        }

        if (downloadSR == 0.0) avgThrput = 0L;
        else {
            avgThrput = (finisBytes - startBytes) * 8L / 1000L / (finishTime / 1000L - startTime / 1000L);
        }
        Logger.d("AvgDL " + avgThrput + " " + finisBytes + " " + finishTime + " " + startBytes + " " + startTime);
